...
func detalhes(midia midia:Midia) {
    
    switch midia {
    case let umaMusica as Musica:
        return print("\(midia.nome) de \(midia.artista)")
    case let umFilme as Filme:
        return print("\(midia.nome) de \(midia.diretor) ")
    default:
        return print("Ops, mídia não reconhecida!")
    }
}

let doIWannaKnow = Musica(nome: "Música", artista: "Arctic Monkeys")
let suicideSquad = Filme(nome: "Filme", diretor: "David Ayer")

detalhes(midia: doIWannaKnow)
detalhes(midia: suicideSquad)
